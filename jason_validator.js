...
function purgeSpaces(string){
  ...
  button.addEventListener('click',function(){
    ...
    var pattern=/^\{(?:\"(?:[a-zA-Z_]+_*\d*[a-zA-Z]*)+\"\:(?:(?:(?:(?:true)?|(?:false)?)|\d*)|(?:\")(?:[^\"])+(?:\"))){1}(?:,\"(?:[a-zA-Z_]+_*\d*[a-zA-Z]*)+\"\:(?:(?:(?:(?:true)?|(?:false)?)|\d*)|(?:\")(?:[^\"\{\}])+(?:\")))*\}$/;
    if(pattern.test(testString)){
      output.innerHTML="The input JSON string is valid JSON \r\r\r"+
      pattern.exec(testString);
    } else{
